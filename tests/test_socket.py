"""
Socket reader tests for pygnssutils - uses dummy socket class
...
class SocketTest(unittest.TestCase):
    ...
    def testSocketStub(self):
        ...
        raw = None
        stream = DummySocket()
        gnr = GNSSReader(stream, bufsize=1024, protfilter=7)
        buff = gnr._stream.buffer  # test buffer getter method
        i = 0
        for (raw, parsed) in gnr.iterate():
            if raw is not None:
